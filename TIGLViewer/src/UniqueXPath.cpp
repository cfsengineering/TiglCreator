...
int cpcr::UniqueXPath::walkSafely(std::string type) {
    ...
    if( this->getFirstElementType() != type){
        throw TIGLViewerException("UniqueXPath::walkSafely: the first element of the xpath: " + this->toString() + " is not of the required type, required type: " + type + " actual time: " + this->getFirstElementType() );
    }
